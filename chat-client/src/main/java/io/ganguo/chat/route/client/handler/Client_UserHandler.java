...
public class Client_UserHandler extends IMHandler<IMRequest> {
    ...
    private Logger logger = LoggerFactory.getLogger(Client_UserHandler.class);
    ...
    private void onLoginSuccess(final IMConnection connection, IMRequest request) {
        ...
        final String login_id = loginDTO.getLogin().getAccount();
        List<Message> list = new ArrayList<Message>();
        list = messageService.GetOfflineMessage(login_id);
        if( list != null){
            for(int i = 0 ; i < list.size() ; i++){
                System.out.println("離線訊息是 : " + list.get(i).getMessage());
            }
            messageService.RemoveOfflineMessage(login_id);
        }
        new Thread() {
            @Override
            public void run() {
